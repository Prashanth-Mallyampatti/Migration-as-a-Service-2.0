...
MIG_ANSIBLE = "/root/Migration-as-a-Service-2.0/src/southbound/ansible/"
MIG_INPUT_PYTHON = "/root/Migration-as-a-Service-2.0/src/northbound/validation_scripts/"
MIG_LOGIC_PYTHON = "/root/Migration-as-a-Service-2.0/src/logiclayer/parser_scripts/"
MIG_INFRA_LOG = "/root/Migration-as-a-Service-2.0/var/logs/infrastructure.log"

dir_exists = os.path.exists(MIG_INFRA_LOG)
if not dir_exists:
  os.system("touch " + str(MIG_INFRA_LOG))

dir_exists = os.path.exists("/root/Migration-as-a-Service-2.0/var/logs/event_handler.log")
if not dir_exists:
  os.system("touch /root/Migration-as-a-Service-2.0/var/logs/event_handler.log")

# Create log file
logging.basicConfig(filename="/root/Migration-as-a-Service-2.0/var/logs/event_handler.log", level=logging.INFO)
...
notifier_infra.add_watch('/root/Migration-as-a-Service-2.0/src/northbound/config_files/infrastructure')
...
for event in notifier_infra.event_gen():
    if event is not None:
        ...
        if 'IN_CREATE' in event[1]:
             ...
             dir_exists=os.path.exists("/root/Migration-as-a-Service-2.0/etc/" + str(dir_name[0]))
             if not dir_exists:
                os.system("mkdir /root/Migration-as-a-Service-2.0/etc/" + str(dir_name[0]))
                os.system("mkdir /root/Migration-as-a-Service-2.0/etc/" + str(dir_name[0]) + "/VM_templates")
             ...
             yaml_file = "/root/Migration-as-a-Service-2.0/src/northbound/config_files/infrastructure/" + str(tenant)
            ...
            # Create infrastructure on cloud 1
             if continue_flag:
                exit_status = os.system("ansible-playbook " + str(MIG_ANSIBLE) + "create_containers_C1.yml -i " + str(MIG_ANSIBLE) + "inventory --extra-vars 'tenant_name=" + str(dir_name[0]) + "' -v >> " + str(MIG_INFRA_LOG))
                #print exit_status
                if exit_status!= 0:
                  continue_flag = False
                  if log_enable:
             ...
             # Create the required files for building infrastructure
             if continue_flag:
                exit_status = os.system("python3 " + str(MIG_LOGIC_PYTHON) + "populate_container_pid.py " + str(dir_name[0]) + " " + str(dir_name[0]) + "c1.yml")
                if exit_status!= 0:
                   continue_flag = False
                   if log_enable:
                      logging.error(' ' + str(datetime.datetime.now().time()) + ' ' + 'FAILED to create infrastructure for tenant ' + str(dir_name[0]))
             ...
             # Create infrastructure on cloud 1
             if continue_flag:
                exit_status = os.system("ansible-playbook " + str(MIG_ANSIBLE) + "create_containers_C2.yml -i " + str(MIG_ANSIBLE) + "inventory --extra-vars 'tenant_name=" + str(dir_name[0]) + "' -v >> " + str(MIG_INFRA_LOG))
                #print exit_status
                if exit_status!= 0:
                  continue_flag = False
                  if log_enable:
             ...
             # Create the required files for building infrastructure
             if continue_flag:
                exit_status = os.system("python3 " + str(MIG_LOGIC_PYTHON) + "populate_container_pid.py " + str(dir_name[0]) + " " + str(dir_name[0]) + "c2.yml")
                if exit_status!= 0:
                   continue_flag = False
                   if log_enable:
                      logging.error(' ' + str(datetime.datetime.now().time()) + ' ' + 'FAILED to create infrastructure for tenant ' + str(dir_name[0]))
